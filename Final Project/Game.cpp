...
bool Game::isValidPickupList(const string& pickupList, 
    ...
    if ((int)pickupList.length() > ELEVATOR_CAPACITY) {
        return false;
    }
    ...
    for (int i = 0; i < (int)pickupList.length(); i++) {
        ...
        if (index >= floor.getNumPeople()) {
            return false;
        }
    }

    for (int i = 0; i < (int)pickupList.length(); i++) {
        for (int j = i + 1; j < (int)pickupList.length(); j++) {
    ...
    int firstIndex = pickupList[0] - '0';
    int direction = floor.getPersonByIndex(firstIndex).getTargetFloor()
                    - pickupFloorNum;

    for (int i = 1; i < (int)pickupList.length(); i++) {
        int index = pickupList[i] - '0';
        int personDir = floor.getPersonByIndex(index).getTargetFloor()
                        - pickupFloorNum;
        if ((direction > 0 && personDir <= 0) ||
            (direction < 0 && personDir >= 0)) {
            return false;
        }
    }
