...
void Building::spawnPerson(Person newPerson){
    int floor = newPerson.getCurrentFloor();
    int request = newPerson.getTargetFloor() - newPerson.getCurrentFloor();
    floors[floor].addPerson(newPerson, request);
}
...
void Building::update(Move move){
    ...
    if (move.isPickupMove()) {
        ...
        int elevFloor = elevators[move.getElevatorId()].getCurrentFloor();
        floors[elevFloor].removePeople(pickupList, move.getNumPeopleToPickup());
    }
